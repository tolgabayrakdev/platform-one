...
export default class CommentService {
    ...
    async createComment(postId, userId, content, parentCommentId = null) {
       ...
       RETURNING id, content, created_at, user_id, parent_comment_id`,
        ...
        const userResult = await pool.query('SELECT id, first_name, last_name FROM users WHERE id = $1', [
            comment.user_id
        ]);
        ...
        if (postOwnerResult.rows.length > 0) {
            ...
            if (parentCommentId) {
                ...
                const parentCommentResult = await pool.query('SELECT user_id FROM comments WHERE id = $1', [
                    parentCommentId
                ]);
